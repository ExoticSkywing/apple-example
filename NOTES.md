...
- 动态岛/锁屏时间使用同一官方 Silent 帧回贴，不再随左侧图标变化。第一次实现误把灵动岛多边形量在了过低位置，导致真机仍显示 Shazam `Listening...`；现已按实际展开胶囊坐标重做，并在 0.8/2.4/4.5 秒逐帧读图确认均为红色静音铃铛 + `Silent`。QA 从视频 Canvas 采样验证三阶段屏幕颜色变化，且灵动岛真实内部采样保持一致。
- 可重复构建脚本：`scripts/build_wallpaper_variant.py`；交付媒体源为 `public/media/action-button-wallpaper-switch.mp4`，页面实际加载采用全新缓存隔离文件名 `action-button-wallpaper-switch-island-fixed-v2.mp4`（880×768、30fps、5秒、150帧），避免 Safari 继续复用旧视频。
